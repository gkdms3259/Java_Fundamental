...
public class CrawlingThread extends Thread{
	...
	@Override
	public void run() {
	    while(true) { //무한루프 돌리려고 써놓은것. 한시간에 한번씩 끌어오려구
		Document doc = null;
		...
		*/
		...
		Elements bodyElements = doc.select(".table-responsive .table tbody tr"); //class가 두개로 된 경우에는 .과 붙이고 하나이고 하부구조로 되어있을때는 뗀다. 
		//System.out.println(bodyElements.size());
			...
			try {
				Thread.sleep(1000*60*60);   //한시간에 한번씩 끌어오게끔 무한루프 잠시 재우는것.
